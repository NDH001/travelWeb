...
from collectiondata.models import UserSightCollection,UserFoodCollection,UserShopCollection
...
class GeneralListView(ListView):
    ...
    def get_queryset(self) -> QuerySet[Any]: 
        queryset = self.get_related_modelset().prefetch_related(self.collection_model_set)
...
class SightsListView(GeneralListView):
    ...
    def get_related_queryset(self):
        return UserSightCollection.objects.filter(
...
class FoodsListView(GeneralListView):
    ...
    def get_related_queryset(self):
        return UserFoodCollection.objects.filter(
        user = self.request.user,
        collection = OuterRef('pk')
        )
...
class ShopsListView(GeneralListView):
    ...
    def get_related_queryset(self):
        return UserShopCollection.objects.filter(
        user = self.request.user,
        collection = OuterRef('pk')
        )
...
def sights_info_view(request,pk,slug):
    ...
    sight_imgs = list(Sights_imgs.objects.select_related('sights').filter(sights=pk).values_list('img_local',flat=True))

    context = {'sights_info':sight_text,
                'titles':titles,
                'descs':descs,
                'info':info,
                'imgs':sight_imgs,
...
class CommentsView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self) -> QuerySet[Any]:
        return Comment.objects.filter(sight=self.kwargs.get('pk')).select_related('user__profile').order_by('-created_on')
